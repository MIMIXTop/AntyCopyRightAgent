from dataclasses import dataclass
from pathlib import Path

import httpx
from aiogram import Bot, Dispatcher
from openai import AsyncOpenAI

from app.agent.history import InMemoryHistoryStore
from app.agent.llm import OpenAIAdapter
from app.agent.models import Message, ToolContext, ToolResult
from app.agent.prompt import load_system_prompt
from app.agent.service import AgentService
from app.agent.tools import ToolRegistry, load_tool_schemas
from app.classroom.client import ClassroomClient, ClassroomService
from app.classroom.tools import register_classroom_tools
from app.config.settings import Settings
from app.core.logging import configure_logging
from app.telegram.handlers import create_router


@dataclass
class Application:
    bot: Bot
    dispatcher: Dispatcher
    http_client: httpx.AsyncClient
    llm_client: AsyncOpenAI

    async def close(self) -> None:
        await self.llm_client.close()
        await self.http_client.aclose()
        await self.bot.session.close()


async def build_application(settings: Settings) -> Application:
    configure_logging()
    http_client = httpx.AsyncClient(
        base_url=settings.CPP_SERVER_URL.rstrip("/"),
        timeout=settings.HTTP_TIMEOUT_SECONDS,
    )
    llm_client = AsyncOpenAI(
        base_url="https://openrouter.ai/api/v1",
        api_key=settings.openai_api_key,
    )
    bot = Bot(token=settings.bot_token_clean)
    registry = ToolRegistry()
    schemas = load_tool_schemas(Path(__file__).parent / "tools")

    async def send_message(context: ToolContext, arguments):
        text = arguments.get("text")
        if not isinstance(text, str) or not text.strip():
            from app.core.errors import ToolArgumentsError

            raise ToolArgumentsError("send_message", "text is required")
        await bot.send_message(chat_id=context.chat_id, text=text)
        return ToolResult(context.call_id, '{"status":"delivered"}')

    async def current_time(context, arguments):
        from app.handlers.get_current_time import get_current_time

        if arguments:
            from app.core.errors import ToolArgumentsError

            raise ToolArgumentsError("get_current_time", "no arguments expected")
        import json

        return ToolResult(context.call_id, json.dumps(get_current_time()))

    async def finish(context, arguments):
        if arguments:
            from app.core.errors import ToolArgumentsError

            raise ToolArgumentsError("finish", "no arguments expected")
        return ToolResult(context.call_id, '{"status":"ok"}', terminal=True)

    registry.register("send_message", send_message, schemas["send_message"])
    registry.register("get_current_time", current_time, schemas["get_current_time"])
    registry.register("finish", finish, schemas["finish"])
    classroom = ClassroomService(ClassroomClient(http_client))
    register_classroom_tools(registry, classroom)

    agent = AgentService(
        llm=OpenAIAdapter(llm_client, settings.LLM_MODEL),
        history=InMemoryHistoryStore(Message("system", load_system_prompt())),
        tools=registry,
        send_message=lambda chat_id, text: bot.send_message(chat_id, text),
    )
    dispatcher = Dispatcher()
    dispatcher.include_router(create_router(agent, settings.CPP_SERVER_URL))
    return Application(bot, dispatcher, http_client, llm_client)